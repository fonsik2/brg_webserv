...
void Response::getReq(Request * req) {
	...
	struct stat	buffer;
		...
		if (req->cgiType == TESTER_CGI || req->cgiType == PHP_CGI) {
				LOGPRINT(INFO, req, ("Response::getReq() : GET - CGI is required to handle that request - Its type is " + ft::to_string(req->cgiType) + " (1 = 42-CGI and 2 = PHP-CGI)"));
				execCGI(req);
...
void Response::postReq(Request * req) {
	...
	struct stat	buffer;
		...
		req->cgiType = getCGIType(req);
		 if (req->cgiType == TESTER_CGI || req->cgiType == PHP_CGI) {
				LOGPRINT(INFO, req, ("Response::postReq() : POST - CGI is required to handle that request - Its type is " + ft::to_string(req->cgiType) + " (1 = 42-CGI and 2 = PHP-CGI)"));
				execCGI(req);
				...
				if ((ret = write(fd, req->_reqBody.c_str(), req->_reqBody.size())) <= 0)
						LOGPRINT(INFO, this, ("Response::postReq() : POST without CGI - write() _reqBody has returned ret = " + ft::to_string(ret)));
				close(fd);
...
void Response::putReq(Request * req)
{
		...
		LOGPRINT(LOGERROR, req, ("Response::putReq() : failed fileFd = " + ft::to_string(fileFd)));
}
