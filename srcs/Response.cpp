...
Response::Response(void) : _resBody(NULL) {
	reset();
}

Response::Response(Client *c) : _resBody(NULL) {
	reset();
	resClient = c;
}
...
void Response::control(Request * req, Server * serv) {
	...
	if (_statusCode != _statusCodeBackup)
		NOCLASSLOGPRINT(LOGERROR, "An error has been detected in  versionControl() - _statusCode is now : " + ft::to_string(_statusCode))
	_statusCodeBackup = _statusCode;
	reqHeadersControl(req);
	if (_statusCode != _statusCodeBackup)
		NOCLASSLOGPRINT(LOGERROR, "An error has been detected in  reqHeadersControl() - _statusCode is now : " + ft::to_string(_statusCode))
	_statusCodeBackup = _statusCode;
	resourceControl(req);
	if (_statusCode != _statusCodeBackup)
		NOCLASSLOGPRINT(LOGERROR, "An error has been detected in  resourceControl() - _statusCode is now : " + ft::to_string(_statusCode))
	_statusCodeBackup = _statusCode;
	methodControl(req, serv);
	if (_statusCode != _statusCodeBackup)
		NOCLASSLOGPRINT(LOGERROR, "An error has been detected in  methodControl(req, () - _statusCode is now : " + ft::to_string(_statusCode))
	_statusCodeBackup = _statusCode;
	authControl(req);
	if (_statusCode != _statusCodeBackup)
		NOCLASSLOGPRINT(LOGERROR, "An error has been detected in  authControl() - _statusCode is now : " + ft::to_string(_statusCode))
	...
	if (!(_resBody) && !(_resFile.empty()))
	{
		...
		else if ((fileFd = open(_resFile.c_str(), O_RDONLY)) != -1)
		{
			while ((retRead = read(fileFd, fileBuf, 4096)) != 0)
			{
				if (retRead == -1)
				{
					LOGPRINT(LOGERROR, this, ("Response::setBody() : read() body file failed | Bytes copied = " + ft::to_string(saveOffset)));
					break ;
				}
			...
			if (retRead == 0)
				LOGPRINT(INFO, this, ("Response::setBody() : read() body file returned 0 | Bytes copied = " + ft::to_string(saveOffset)));
...
void Response::format(void) {
	...
	formatedResponse.append(" " + ft::to_string(_statusCode) + " ");
...
std::string const Response::logInfo(void) {
	...
	ret = "Response | Destination : Client from port " + ft::to_string(resClient->port) + " (socket n°" + ft::to_string(resClient->acceptFd) + ") | Method : " + resClient->req.method + " | Response Status : " + ft::to_string(_sendStatus) + " | Response Code : " + ft::to_string(_statusCode);
	return (ret);
...
void Response::showRes(void) {
	...
	std::cout << indent << "Status Code : " << ft::to_string(_statusCode) << std::endl;
...
void Response::showFullHeadersRes(void) {
	...
	std::cout << std::endl;
	std::cout << indent << "Content-Length : " << ft::to_string(contentLength) << std::endl;
	...
	int x =  contentLength;
	std::cout << indent << "_resBody Size : " << ft::to_string(x) << std::endl;
