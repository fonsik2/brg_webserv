...
void Server::readClientRequest(Client *c) {
	...
	if (!(recvCheck) || recvRet == 0)
	{
		...
		if (recvRet == 0)
		{
			LOGPRINT(DISCONNECT, c, ("Server::readClientRequest : recv() returned 0 : The client (port " + ft::to_string(c->port) + ") has closed its connection"));
		}
		...
		LOGPRINT(INFO, c, ("Server::readClientRequest() : recv() has read " + ft::to_string(c->req._reqBody.size()) + " bytes"));
...
void Server::writeClientResponse(Client *c) {
	...
	if (c->res._sendStatus == Response::PREPARE || c->res._sendStatus == Response::ERROR) {
		if (c->res._sendStatus == Response::ERROR)
			LOGPRINT(LOGERROR, c, ("Server::writeClientResponse() : sendStatus = ERROR - Code = " + ft::to_string(c->res._statusCode)));
		setClientResponse(c);
	}
...
int Server::sendBytes(Client *c, char *toSend, long bytesToSend)
{
	...
	while (bytesSent < totalBytes) {
		...
		bytesSent += retSend;
		bytesToSend -= retSend;
		NOCLASSLOGPRINT(INFO, ("Cumulated bytes sent : " + ft::to_string(bytesSent)));
	}
	if (bytesToSend != 0) {
		LOGPRINT(LOGERROR, c, ("Server::writeClientResponse() : send() not complete --> Bytes sent : "
		+ ft::to_string(bytesSent) + " (total = " + ft::to_string(totalBytes) + ")"));
		return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}
...
std::string const Server::logInfo(void) {
	std::string ret;
	ret = name + " | " + "Listening Port " + ft::to_string(port) + " | Clients connected : " + ft::to_string(clients.size());
	return (ret);
}
