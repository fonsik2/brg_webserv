...
std::string mapToStr(std::map<int, std::string> map, char sep) {
 ...
 std::map<std::string, std::string> Request::mapReqHeaders(void) {
	...
	ret["Content-Length"] = ft::to_string(contentLength);
...
void Request::parseUriQueries() {
	...
	if (uriQueries.size() > 1024 && client != NULL)
...
void Request::parseBody()
{
	if (client->recvStatus == Client::BODY)
	{
		...
		{
			LOGPRINT(REQERROR, client, ("Server::readClientRequest() : Error : REQUEST_ENTITY_TOO_LARGE_413 - Max = " + ft::to_string(reqLocation->max_body)));
...
void Request::parseChunkedBody() {
	...
	while (42) {
		...
		if (chunkLineBytesSize > 0) {
			...
			if (separator != std::string::npos)
				_reqBody.erase(separator, 2);
			else if (!_reqBody.empty() && *_reqBody.rbegin() == '0')
...
void Request::parseSingleBody() {
	...
	if (bodySize == contentLength)
	{
		...
		LOGPRINT(INFO, this, ("Request::parseSingleBody() : Body is fully received ! Header Content-Length = " \
		 + ft::to_string(contentLength) + " should be equal to Body length, which is : " + ft::to_string(bodySize)));
	}
...
void Request::checkBody() {
	...
	if (contentLength > 0 || (transferEncoding.size() && transferEncoding[0] == "chunked")) {
		...
		if (transferEncoding[0] == "chunked") {
			...
			LOGPRINT(INFO, this, ("Request::checkBody() : Body Content-Length = " + ft::to_string(contentLength)));
		}
	} else
		client->recvStatus = Client::COMPLETE;
...
void Request::parse(std::vector<Location*> locations) {
	/* LOGPRINT(LOGERROR, this->client, "1 -------> " + ft::to_string(this->client->res._statusCode)); */
	parseRequestLine();
	/* LOGPRINT(LOGERROR, this->client, "2 -------> " + ft::to_string(this->client->res._statusCode)); */
	parseUriQueries();
	/* LOGPRINT(LOGERROR, this->client, "3 -------> " + ft::to_string(this->client->res._statusCode)); */
	parseFile(locations);
	/* LOGPRINT(LOGERROR, this->client, "4 -------> " + ft::to_string(this->client->res._statusCode)); */
	parseHeaders();
	/* LOGPRINT(LOGERROR, this->client, "5 -------> " + ft::to_string(this->client->res._statusCode)); */
	checkBody();
	/* LOGPRINT(LOGERROR, this->client, "6 -------> " + ft::to_string(this->client->res._statusCode)); */
}
...
std::string const Request::logInfo(void) {
	std::string ret;
	ret = "Request | From Client with Socket : " + ft::to_string(client->acceptFd) + " | Method : " + method + " | URI : " + uri;
	return (ret);
}
...
void Request::showFullHeadersReq(void) {
	...
	std::cout << indent << "_currentParsedReqBodyLength = " << ft::to_string(_currentParsedReqBodyLength) << std::endl;
	
	int x =  _reqBody.size();
	std::cout << indent << "_reqBody Size : " << ft::to_string(x) << std::endl;
